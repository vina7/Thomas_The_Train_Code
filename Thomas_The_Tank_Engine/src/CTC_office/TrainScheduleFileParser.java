		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				ScheduleInfo schedule = new ScheduleInfo(info[0], info[1], Double.parseDouble(info[2]),
						tracks.getBlockNum(info[1], info[0]));
			...
			if (TrainSchedule.get(0).getLine().equals("Green")) {
				...
				loop: for (int i = 0; i < tracks.getGreenTrack().size(); i++) {
					if (tracks.getGreenTrack().get(i).getSection().equals("YY")) {
						startLine = tracks.getGreenTrack().get(i);
						break loop;
					}
				}
				trainlist.addTrain(color, trainlist.getRedTrain().size() + trainlist.getGreenTrain().size(),
						(int) (startLine.getSpeedLimit()),
						(int) (((Math.pow((startLine.getSpeedLimit() * .447), 2)) / (2 * 1.2)) * 3.3), "Head", "-1",
						"YY", startLine.getBlockNum(), (int) (startLine.getBlockLen()), startLine.getBlockGrade(),
						(int) (startLine.getSpeedLimit()), startLine.getElevation(), false, "Good", "PIONEER",
						TrainSchedule.get(0).getBlockNum(), TrainSchedule, false, false, 0, 0);
				map.updateColor("YY", "-1", "Green", color);

			} else {
				TrackBlock startLine = tracks.getRedTrack().get(0);
				loop2: for (int i = 0; i < tracks.getRedTrack().size(); i++) {
				...
				trainlist.addTrain(color, trainlist.getRedTrain().size() + trainlist.getGreenTrain().size(),
						(int) (startLine.getSpeedLimit()),
						(int) (((Math.pow((startLine.getSpeedLimit() * .447), 2)) / (2 * 1.2)) * 3.3), "Head", "-1",
						"U", startLine.getBlockNum(), (int) (startLine.getBlockLen()), startLine.getBlockGrade(),
						(int) (startLine.getSpeedLimit()), startLine.getElevation(), false, "Good", "SHADYSIDE",
						TrainSchedule.get(0).getBlockNum(), TrainSchedule, false, false, 0, 0);
