...
public class SetDestination {
	...
	public AllTrains setManualDestination(int ID, int BlockNum, VerifyManualMode mode, AllTrains train,
		...
		if (mode.verify()) {
			...
			if (!((Blocks.getInfrastructure(BlockNum, train.getTrainSchedule(ID).get(0).getLine())
					.equalsIgnoreCase(" "))
					|| (Blocks
							.getInfrastructure(BlockNum,
									train.getTrainSchedule(ID).get(0)
											.getLine())
							.equalsIgnoreCase("SWITCH"))
					|| (Blocks.getInfrastructure(BlockNum, train.getTrainSchedule(ID).get(0).getLine())
							.equalsIgnoreCase("RAILWAY CROSSING"))
					|| (Blocks.getInfrastructure(BlockNum, train.getTrainSchedule(ID).get(0).getLine())
							.equalsIgnoreCase("UNDERGROUND")))) {
				train.setStation(Blocks.getInfrastructure(BlockNum, train.getTrainSchedule(ID).get(0).getLine()), ID,
						train.getTrainSchedule(ID).get(0).getLine());
				List<ScheduleInfo> stations = train.getTrainSchedule(ID);
				for (int y = 0; y < stations.size(); y++) {
					if (stations.get(y).getStation().equalsIgnoreCase(
							Blocks.getInfrastructure(BlockNum, train.getTrainSchedule(ID).get(0).getLine()))) {
						train.setStationNum(y, ID, train.getTrainSchedule(ID).get(0).getLine());
					}
				}
			}
			// System.out.println(train.getDestBlock(ID,
			// train.getTrainSchedule(ID).get(0).getLine()));
	...
	private AllTrains setAutomaticDestination(int ID, int BlockNum, AllTrains train, AllTrackBlock Blocks,
			boolean reversedirection, CTCandTrackControllerInterface TCinterface) {
		List<ScheduleInfo> stations = train.getTrainSchedule(ID);
		if (stations.get(train.getStationNum(ID, train.getTrainSchedule(ID).get(0).getLine()))
				.getBlockNum() == BlockNum) {
			TCinterface.setTransponder(ID, true);
			if (train.getStationNum(ID, train.getTrainSchedule(ID).get(0).getLine()) + 1 < stations.size()) {
				train.setDestBlock(stations
						.get(train.getStationNum(ID, train.getTrainSchedule(ID).get(0).getLine()) + 1).getBlockNum(),
						ID, train.getTrainSchedule(ID).get(0).getLine());
				train.setStation(stations.get(train.getStationNum(ID, train.getTrainSchedule(ID).get(0).getLine()) + 1)
						.getStation(), ID, train.getTrainSchedule(ID).get(0).getLine());
				train.setStationNum(train.getStationNum(ID, train.getTrainSchedule(ID).get(0).getLine()) + 1, ID,
						train.getTrainSchedule(ID).get(0).getLine());
			...
			train.setDestBlock(
					stations.get(train.getStationNum(ID, train.getTrainSchedule(ID).get(0).getLine())).getBlockNum(),
					ID, train.getTrainSchedule(ID).get(0).getLine());
		}
		return train;
	}
	...
	public AllTrains checkPastDestionation(int ID, AllTrains train, AllTrackBlock Blocks, boolean reversedirection,
			CTCandTrackControllerInterface TCinterface) {
		if (train.getBlockNum(ID, train.getTrainSchedule(ID).get(0).getLine()) == train.getDestBlock(ID,
				train.getTrainSchedule(ID).get(0).getLine())) {
			train = setAutomaticDestination(ID, train.getDestBlock(ID, train.getTrainSchedule(ID).get(0).getLine()),
					train, Blocks, reversedirection, TCinterface);
		}
		return train;
	}
}
