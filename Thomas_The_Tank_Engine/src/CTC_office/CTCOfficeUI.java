...
public class CTCOfficeUI {
	 ...
	 */
	public CTCOfficeUI(AllTrains Train, AllTrackBlock Blocks, CTCOffice CTC, TrackCircuit circuit, CTCMapUI map) {
		this.Train = Train;
		this.Blocks = Blocks;
		this.temp = CTC;
	...
	private void initialize() {
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					if (file.getPath().contains("Red_Line_Schedule.csv")) {
						...
						circuit.makeNewTrain(newtrain.getID(), newtrain.getBlockGrade(), 0, newtrain.getSpeed(),
								newtrain.getAuthority(), "Red", newtrain.getBlockNum(), false);
						CTCTCint.makeNewTrain(newtrain.getID(), newtrain.getBlockSpeedLim(),
								newtrain.getTrainSchedule().get(0).getStation(), false, 7);
					} else if (file.getPath()
							.contains("Green_Line_Schedule.csv")) {
						List<Trains> alltrains = new ArrayList<Trains>();
						alltrains.addAll(Train.getRedTrain());
						alltrains.addAll(Train.getGreenTrain());
						Line.updateTrainList(alltrains);
						...
						circuit.makeNewTrain(newtrain.getID(), newtrain.getBlockGrade(), 0, newtrain.getSpeed(),
								newtrain.getAuthority(), "Green", newtrain.getBlockNum(), false);
						CTCTCint.makeNewTrain(newtrain.getID(), newtrain.getBlockSpeedLim(),
								newtrain.getTrainSchedule().get(0).getStation(), false, 2);
					}
		...
		btnBrakeRail.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if ((Line1.equalsIgnoreCase("Red") || Line1.equalsIgnoreCase("Green"))
						&& (Integer.parseInt(BlockNumber) < 152 && Integer.parseInt(BlockNumber) > 0)) {
		...
		btnNewButton_1.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (returnVal == JFileChooser.APPROVE_OPTION) {
					...
					if (file.getPath().contains("Red_Track_Layout.csv")) {
						Blocks = switchinterface.updateBlocks(Blocks, "Red");
						switchs.updateBlocks(Blocks);

					} else if (file.getPath()
							.contains("Green_Track_Layout.csv")) {
