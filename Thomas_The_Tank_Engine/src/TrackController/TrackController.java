...
/**
 * 
 * @author Kaitlyn Carey This file is was part of the system prototype. It is
 *         used as a place holder for the track controller.
 *
 */
/*
 * Track Controller. The system will be controlled by 4 TCs, each controlling a
 * quadrant of the track.
 */
public class TrackController {
	...
	public static void moveAllNeededSwitches() {
		...
		for (int i = 0; i < activetrains.size(); i++) {
			...
			id = (activetrains.get(i)).getID();
			currBlockNum = allTrains.getBlockNum(id, line); // block number
															// where train is
			currSection = allTrackBlock.getSection(currBlockNum, line);
			// GET PREV SECTION

			if (greenSwitchLocationBlockNums.contains(currBlockNum)
					|| redSwitchLocationBlockNums.contains(currBlockNum)) { // if
																			// current
																			// block
																			// is
																			// source
			...
			if (greenSwitchDestinationBlockNums.contains(currBlockNum)
					|| redSwitchDestinationBlockNums.contains(currBlockNum)) { // if
																				// current
																				// block
																				// is
																				// destination
	 ...
	 * 
	 * String line; int id; int blockNum;
	 * 
	 * int switchArrayIndex; Integer[] switchOptions;
	 * 
	 * 
	 * 
	 * AllTrains allTrains = AllTrains.getInstance(false); TrackCircuit
	 * trackCircuit = TrackCircuit.getInstance(false); ArrayList
	 * <TrackCircuitTrainInfo> activetrains = trackCircuit.getActiveTrains();
	 * Switches switches = Switches.getInstance(false);
	 * 
	 * ArrayList <Integer> greenSwitchLocationBlockNums =
	 * switches.getGreenswitchlocations(); ArrayList <Integer[]>
	 * greenSwitchOptions = switches.getGreenswitchoptions(); ArrayList
	 * <Integer> redSwitchLocationBlockNums = switches.getRedswitchlocations();
	 * ArrayList <Integer[]> redSwitchOptions = switches.getRedswitchoptions();
	 * 
	 * //For all active trains, for(int i=0;i<activetrains.size();i++){
	 * 
	 * line = (activetrains.get(i)).getLine(); id =
	 * (activetrains.get(i)).getID(); blockNum = allTrains.getBlockNum(id,
	 * line); //block number where train is
	 * if(greenSwitchLocationBlockNums.contains(blockNum)||
	 * redSwitchLocationBlockNums.contains(blockNum)){ //if the train is at a
	 * switch
	 * 
	 * if(line.equals("Green")){ switchArrayIndex =
	 * greenSwitchLocationBlockNums.indexOf(blockNum); switchOptions =
	 * greenSwitchOptions.get(switchArrayIndex);
	 * 
	 * 
	 * } else if (line.equalsIgnoreCase("Red")){ switchArrayIndex =
	 * redSwitchLocationBlockNums.indexOf(blockNum); switchOptions =
	 * redSwitchOptions.get(switchArrayIndex); }
	 * 
	 * 
	 * 
	 * 
	...
	public static void verifyAllTrainAuthorities() {
		...
		for (int i = 0; i < activetrains.size(); i++) {
			...
			authority = allTrains.getAuthority(id, line); // train authority by
															// distance
			authorityDecrement = authority;
			blockNum = allTrains.getBlockNum(id, line); // block number where
														// train is
			blockNumIncrement = blockNum;

			while (authorityDecrement > 0) {

				blockLen = allTrackBlock.getBlockLen(blockNumIncrement, line); // get
																				// current
																				// block's
																				// length

				authorizedBlocks.add(blockNum); // add current authorized block
												// to list of authorized blocks.
												// If a duplicate, 2 trains have
												// been authorized for the same
												// block. Current train's
												// authority is dangerous.

				for (int j = 0; j < (authorizedBlocks.size() - 1); j++) {
					if (authorizedBlocks.get(j) == authorizedBlocks.get(authorizedBlocks.size() - 1)) { // a
																										// block
																										// is
																										// added
																										// to
				...
				authorityDecrement = authorityDecrement - blockLen; // subtract
																	// current
																	// block
																	// length
																	// from
				...
				blockNumIncrement = blockNumIncrement++; // TO CHECK THE NEXT
															// BLOCK. WILL NOT
															// WORK IF THE NEXT
															// BLOCK IS AFTER A
															// SWITCH!
			...
			if (badAuthority == true) { // fix the authority
				trackCircuit.setAuthority((allTrackBlock.getBlockLen(blockNum, line)), id); // If
																							// another
																							// train
																							// is
																							// authorized
