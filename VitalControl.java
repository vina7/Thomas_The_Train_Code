...
public class VitalControl {
	...
	private	final double	Kp = 6171.56;						//Proportional Gain
	...
	public VitalControl(Controller controller) {
	...
	private double calculatePower(double velocityFeedback, int i) {
		double powerCommand;
		System.out.println("vf"+velocityFeedback+" tv"+targetVelocity);
		newVelocityError[i] = targetVelocity - velocityFeedback;
		
		if(newPowerCommand[i] < MAX_POWER && newPowerCommand[i] > -MAX_POWER) {
